  ...
  describe 'GET /trips' do
    describe 'happy path' do
      it 'returns all trips with their years, numbers, and locations' do
        ...
        expect(trips[0].keys).to eq([:year, :number, :location])
        expect(trips[0][:year]).to eq(2013)
        expect(trips[0][:number]).to eq(13)
        expect(trips[0][:location]).to eq('Dewey Beach')
        expect(trips[1][:year]).to eq(2014)
        expect(trips[1][:number]).to eq(14)
        expect(trips[1][:location]).to eq('Rehoboth Beach')
        expect(trips[2][:year]).to eq(2015)
        expect(trips[2][:number]).to eq(15)
        expect(trips[2][:location]).to eq('VA Beach')
        expect(trips[3][:year]).to eq(2016)
        expect(trips[3][:number]).to eq(16)
        expect(trips[3][:location]).to eq('VA Beach')
      end
    end
  end
end
